 ...
 * @author sadev_vr38
 */
@WebServlet(name = "AdminLoadPayments", urlPatterns = {"/AdminLoadPayments"})
public class AdminLoadPayments extends HttpServlet {
        ...
        if (httpSession.getAttribute("id") != null && httpSession.getAttribute("email") != null) {
            ...
            if (!adminCriteria.list().isEmpty()) {
                ...
                if (!criteria.list().isEmpty()) {
                    ...
                    List<Payments> payments = criteria.list();
                    
                    for (Payments payment : payments) {
                        
                        payment.getRentals().getProduct().getOwner().setPassword(null);
                        payment.getRentals().getUser().setPassword(null);
                        
                    }
                    
                    responseJson.add("orders", gson.toJsonTree(payments));
                    
                }else{
                    
                    responseJson.addProperty("message", "No Orders Yet!");
